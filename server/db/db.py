...
from server.control_type import ControlType
...
def get_statuses():
    ...
    data = []
    for r in rows:
        data.append(Status(r['status_name'], r['status_value'], r['status_display'], r['icon']))
    return data
...
def get_status(name):
    ...
    stat = Status(row['status_name'], row['status_value'], row['status_display'], row['icon'])
    print(row['icon'])
    stat.icon = row['icon']
    print(stat.icon)
    return stat
